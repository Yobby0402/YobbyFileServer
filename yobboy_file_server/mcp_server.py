...
def _safe_int_arg(value: Any, field: str, default: int = 0) -> int:
    try:
        return int(value if value is not None and value != "" else default)
    except (TypeError, ValueError):
        raise MCPError("VALIDATION_ERROR", f"{field} 参数必须是整数")
...
class YFSMCPServer:
    def __init__(self, todo_storage_path: Optional[str], root_dir: str):
        ...
        self.erp_manager = LocalERPManager()
    ...
    def _build_tools(self) -> Dict[str, Dict[str, Any]]:
        return {
            ...
            "kb_rebuild_entry": {
                "description": "rebuild knowledge entry",
                "inputSchema": {
                    "type": "object",
                    "required": ["path"],
                    "properties": {"path": {"type": "string", "minLength": 1}},
            ...
            "kb_index_status": {
                ...
                "handler": self.kb_index_status,
            },
            "kb_list_snippets": {
                "description": "list snippet knowledge items",
                "inputSchema": {
                    "type": "object",
                    "properties": {},
                    "additionalProperties": False,
                },
                "handler": self.kb_list_snippets,
            },
            "kb_add_snippet": {
                "description": "add snippet knowledge",
                "inputSchema": {
                    "type": "object",
                    "required": ["text"],
                    "properties": {
                ...
                "handler": self.kb_add_snippet,
            },
            "kb_update_snippet": {
                "description": "update snippet knowledge",
                "inputSchema": {
                    "type": "object",
                    "required": ["id", "text"],
                    "properties": {
                ...
                "handler": self.kb_update_snippet,
            },
            "kb_remove_snippet": {
                "description": "remove snippet knowledge",
                "inputSchema": {
                    "type": "object",
                    "required": ["id"],
                ...
                "handler": self.kb_remove_snippet,
            },
            "kb_retrieve": {
            ...
            "erp_get_context": {
                "description": "获取本地离线 ERP 的上下文快照，供 AI 回答库存、BOM、工单与追踪问题",
                "inputSchema": {
                    "type": "object",
                    "required": ["query"],
                    "properties": {
                        "query": {"type": "string", "minLength": 1},
                        "active_page": {"type": "string"},
                        "focused_item_id": {"type": "integer", "minimum": 0},
                        "focused_bom_id": {"type": "integer", "minimum": 0},
                        "focused_work_order_id": {"type": "integer", "minimum": 0},
                        "trace_keyword": {"type": "string"},
                        "trace_batch_no": {"type": "string"},
                    },
                    "additionalProperties": False,
                },
    ...
    def kb_get_entry(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        ...
        meta = knowledge_store.get_meta(path, root_dir=self.root_dir)
    ...
    def kb_rebuild_entry(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        path = _safe_len_text(args.get("path"), _MAX_TEXT_FIELD, "path").strip()
        try:
            entry = knowledge_store.rebuild_entry(self.root_dir, path)
        except ValueError as e:
            raise MCPError("VALIDATION_ERROR", str(e))
        return _ok({"entry": entry}, trace_id)

    def kb_index_status(self, _args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        return _ok(knowledge_store.get_index_status(self.root_dir), trace_id)

    def kb_list_snippets(self, _args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        items = knowledge_store.list_snippets(self.root_dir)
        return _ok({"items": items, "count": len(items)}, trace_id)

    def kb_add_snippet(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        text = _safe_len_text(args.get("text"), _MAX_TEXT_FIELD, "text").strip()
        if not text:
            raise MCPError("VALIDATION_ERROR", "text 涓嶈兘涓虹┖")
        title = _safe_len_text(args.get("title") or "", 200, "title")
        note = _safe_len_text(args.get("note") or "", _MAX_TEXT_FIELD, "note")
        tags = args.get("tags")
        if tags is not None and not isinstance(tags, list):
            raise MCPError("VALIDATION_ERROR", "tags 蹇呴』涓哄瓧绗︿覆鏁扮粍")
        try:
            item = knowledge_store.add_snippet(self.root_dir, text, title=title, note=note, tags=tags)
        except ValueError as e:
            raise MCPError("VALIDATION_ERROR", str(e))
        return _ok({"item": item}, trace_id)

    def kb_update_snippet(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        snippet_id = _safe_len_text(args.get("id"), 128, "id").strip()
        text = _safe_len_text(args.get("text"), _MAX_TEXT_FIELD, "text").strip()
        if not snippet_id or not text:
            raise MCPError("VALIDATION_ERROR", "id / text 涓嶈兘涓虹┖")
        title = _safe_len_text(args.get("title") or "", 200, "title")
        note = _safe_len_text(args.get("note") or "", _MAX_TEXT_FIELD, "note")
        tags = args.get("tags")
        if tags is not None and not isinstance(tags, list):
    ...
    def kb_remove_snippet(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        snippet_id = _safe_len_text(args.get("id"), 128, "id").strip()
        if not snippet_id:
            raise MCPError("VALIDATION_ERROR", "id 涓嶈兘涓虹┖")
        knowledge_store.remove_snippet(self.root_dir, snippet_id)
        return _ok({"removed": True, "id": snippet_id}, trace_id)
    ...
    def erp_get_context(self, args: Dict[str, Any], trace_id: str) -> Dict[str, Any]:
        ...
        if not query:
            raise MCPError("VALIDATION_ERROR", "query 不能为空")
        page_context = {
            "active_page": _safe_len_text(args.get("active_page") or "", 80, "active_page").strip(),
            "focused_item_id": _safe_int_arg(args.get("focused_item_id"), "focused_item_id"),
            "focused_bom_id": _safe_int_arg(args.get("focused_bom_id"), "focused_bom_id"),
            "focused_work_order_id": _safe_int_arg(args.get("focused_work_order_id"), "focused_work_order_id"),
            "trace_keyword": _safe_len_text(args.get("trace_keyword") or "", 120, "trace_keyword").strip(),
            "trace_batch_no": _safe_len_text(args.get("trace_batch_no") or "", 120, "trace_batch_no").strip(),
        }
        data = self.erp_manager.build_ai_context(query, page_context=page_context)
