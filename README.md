...
**📋 ToDo AI 安全执行链路**
- **二次确认机制**：AI 生成待办变更后先进入确认弹窗，支持“校验 JSON”后再应用，避免误操作
- **待执行持久化**：待确认操作可在刷新后恢复；关闭弹窗不会丢失，AI 按钮会红色闪烁提醒
- **幂等与令牌保护**：写操作结合 `confirm_token` 与 `idempotency_key`，避免重复应用
- **人类可读编辑器**：确认弹窗内将操作类型映射为可读文案，项目/任务引用显示名称而非纯 ID

**🎨 Draw.io AI 与自修复反馈循环**
- **Phase 1 工具**：`drawio_validate_xml`、`drawio_summarize_xml`、`drawio_diff_summary`
- **Phase 2 工具**：`drawio_repair_xml` 自动修复 XML
- **自动反馈循环**：生成失败时基于校验问题自动重试修复；最终增加收口校验，确保后续 summarize/diff 使用稳定 XML
- **布局质量校验**：新增重叠与越界检测（如 `LAYOUT_OVERLAP`、`LAYOUT_OUT_OF_BOUNDS`）用于发现“可解析但不可读”的图

**🛠 体验优化（ToDo / 产品对比）**
- **汇报视图**：粘土拟态主题下恢复表格行选中高亮；**「导出汇报 Excel」** 与 **「全屏查看」** 移至顶部工具栏（仅汇报视图显示）
- **快捷控制展示范围**：在汇报视图中，可右键**项目名称**或**任务简述**单元格，经确认后关闭「显示在汇报页」（`show_in_report`），在编辑项目/任务中可重新勾选恢复
- **产品对比**：玻璃/粘土拟态下强化产品勾选框的选中样式，更易区分已选与未选

### 🌟 R1.3 版本功能更新 (2026-04-02)

**🔌 串口助手全面升级**
- **浏览器本地串口共享**：打开串口页后，可直接把当前浏览器已授权的本地串口共享到服务器，其他设备无需安装任何客户端程序即可接入
- **统一远程串口目录**：服务器串口（PySerial）与浏览器共享串口（Web Serial）会同时出现在远程串口列表中，支持多客户端同步查看和发送
- **持续日志与历史分页**：共享串口和服务器串口均支持历史记录管理，页面默认只加载最近一段内容，更多日志按需继续读取，避免长会话拖慢页面
- **HTTPS 兼容模式**：可保留主站 HTTP，同时给串口助手单独启用 HTTPS 入口，满足 Web Serial 的安全上下文要求
- **离线可靠性增强**：内置本地 `socket.io.min.js`，解决外网不可达时远程串口初始化失败的问题

**🎨 主题与页面稳定性**
- **首屏主题预加载**：在页面首帧应用主题色与拟态模式，减少文件服务器、登录页、模型查看页、串口页的背景闪烁
- **拟态效果稳定**：修复页面切换、进入子目录和局部刷新时先退回默认样式再恢复拟态的问题

**🔧 Git 与协作管理增强**
- **GUI 生成 SSH 密钥**：可直接在桌面端生成托管 SSH 密钥、公钥自动展示，便于快速配置 Git 服务器
- **ToDo 归档与本周计划**：项目支持归档，任务新增“本周计划”字段并参与持久化与导出，适合持续汇报场景

### 🌟 R1.2.1 版本更新 (2026-01-29)

**🔧 串口助手与持续日志**
- 修复部分环境下 `serial` 包冲突导致持续日志无法启动（改为从 `serial.serialwin32` / `serial.serialposix` 导入）
- 修复安全上下文提示中 localhost 链接端口写死为 5000 的问题（随当前页面端口变化）
- 修复持续日志「启动时间」时区显示偏差；列表增加记录时长、记录长度与自动刷新，支持弹窗查看日志

**📊 产品对比 / 📋 ToDo**
- 修复打开对比文件后顶部文件选择框仍显示「请选择」的问题
- 待完成概览：无截止日期的任务不再展示卡片（统计中的「未设置」数量仍保留）

### 🌟 R1.2 版本功能更新 (2026-01-20)

**🔧 Git版本控制集成**
- **Git工作区管理**：独立的Git视图，支持管理Git工作目录中的所有仓库
- **仓库操作**：克隆、拉取、推送、同步、提交、打Tag等完整的Git操作
- **远程仓库浏览**：支持从Git服务器（如Gitolite）获取仓库列表并选择克隆
- **文件编辑**：Git仓库中的文件支持在线编辑和Markdown分屏预览
- **提交历史**：查看仓库的提交历史记录
- **分支管理**：支持分支切换和查看
- **安全配置**：Git服务器配置（SSH密钥、服务器地址等）仅在PyQt5 GUI中设置，防止数据外泄
- **本地集成**：支持打开仓库到系统文件浏览器或外部软件（如VSCode），仅限本机访问

### 🌟 R1.1 版本功能更新 (2026-01-13)

**📋 ToDo 任务管理系统增强**
- **汇报表格Excel导出**：支持将汇报视图导出为Excel文件，自动根据当前页面列宽调整Excel列宽，智能行高调整确保内容完整显示，支持Excel筛选功能
- **会议记录（笔记）功能**：独立的笔记管理功能，支持按日期创建和管理笔记，Markdown编辑和实时预览（左右分屏模式），笔记列表支持搜索、排序和筛选
...
- 📋 **ToDo 任务管理系统 v2**：完整的项目-任务层级结构，支持Excel导出、汇报视图、汇报表格Excel导出、会议记录功能
...
- 🔧 **Git版本控制集成**：Git工作区管理、仓库操作、远程仓库浏览、文件编辑、提交历史、分支管理
...
- **共享本地串口**：浏览器可将本地 Web Serial 串口共享到服务器，其他终端从远程串口列表直接接入
- **持续日志捕获**：为选定串口开启后台 JSONL 日志，可在页面一键载入或持续监控
- **历史分页加载**：默认只展示最近一段日志，旧记录按需继续加载，适合长时间运行场景
...
#### 🔧 Git版本控制集成（R1.1）
- **Git工作区管理**：独立的Git视图，专门用于管理Git工作目录中的所有仓库
- **仓库列表**：自动扫描Git工作目录，显示所有仓库及其状态（分支、远程地址、工作区状态）
- **远程仓库浏览**：支持从Git服务器（如Gitolite）获取仓库列表，选择并克隆到本地
- **仓库操作**：
  - **拉取 (Pull)**：从远程仓库下载最新更改
  - **推送 (Push)**：将本地提交上传到远程仓库（自动检查未提交更改）
  - **同步 (Sync)**：先拉取再推送，确保本地和远程完全同步
  - **提交 (Commit)**：将工作区更改保存到本地仓库，支持提交信息输入
  - **打Tag**：为特定提交创建标签，用于标记版本
  - **历史查看**：查看仓库的提交历史记录
  - **分支管理**：切换和查看分支
- **文件编辑**：
  - Git仓库中的文件支持在线编辑（CodeMirror语法高亮）
  - Markdown文件支持分屏预览模式
  - 编辑后自动检测未提交更改状态
- **仓库管理**：
  - **克隆仓库**：从远程服务器克隆仓库到本地（支持选择仓库或手动输入URL）
  - **删除仓库**：删除本地仓库（仅限本机访问）
  - **打开文件浏览器**：在系统文件浏览器中打开仓库（仅限本机访问）
  - **用外部软件打开**：使用配置的外部软件（如VSCode）打开仓库（仅限本机访问）
- **安全配置**：
  - Git服务器配置（SSH密钥、服务器地址、认证方式等）仅在PyQt5 GUI中设置
  - 支持在 GUI 中直接生成新的 RSA SSH 密钥并查看公钥内容
  - 防止敏感信息在Web界面泄露
  - 支持SSH密钥和HTTPS认证方式
- **状态监控**：
  - 自动检测工作区状态（修改、暂存、未跟踪文件）
  - 实时显示分支、远程地址、工作区状态
  - 30秒自动刷新仓库状态
- **调试工具**：内置Git命令调试工具，支持执行Git命令并查看输出
...
git_enabled = false               # Git功能开关（true=启用，false=禁用）
git_workdir =                     # Git工作目录（启用Git功能时设置）
git_external_app_path =           # 外部软件路径（如VSCode的code.exe，用于打开仓库）
log_level = info                  # 应用日志等级：debug / info / warning / error（默认 info）
```

**日志**：
- 业务日志写入程序目录下 `logs/app.log`（按天轮转）；HTTP 访问摘要写入 `logs/access.log`。
- 环境变量 `YOBBY_LOG_LEVEL`（如 `DEBUG`）若设置，将**覆盖** `config.ini` 中的 `log_level`。
- 在终端直接运行 `python main.py run` 时，若标准输出为 TTY，控制台会带等级颜色；由 GUI 子进程捕获时自动使用纯文本，避免乱码。
