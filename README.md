...
- 📊 **Excel导出列宽优化**：缩小"详细描述"列的默认宽度，优化表格显示效果

### 🌟 v3.7 功能

- 📊 **ToDo v2 Excel 导出增强**：
  - **项目阶段和任务类型**：项目支持阶段属性（预研阶段/ES1阶段/PL阶段/MP阶段/完成/暂停/终止），任务支持类型属性（预研任务/研发任务）和结论字段
  - **导出周报功能**：支持导出上周更新的任务和未完成任务，包含"上周进展"和"本周计划"列，支持合并评论和结论
  - **导出预览功能**：新增预览界面，支持在线编辑Excel数据，支持单元格内容编辑、列拖动排序、列宽调整等功能
  - **主题适配**：预览界面支持多主题切换，表格背景和文字颜色随主题自动调整
